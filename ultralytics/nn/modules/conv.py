...
class SPDConv(nn.Module):
    """Space-to-depth downsampling followed by a non-strided convolution.

    Unlike a stride-2 convolution, space-to-depth preserves all four spatial
    sub-pixels by moving them into the channel dimension before projection.
    """

    def __init__(self, c1, c2, k=3, block_size=2, act=True):
        super().__init__()
        if block_size < 2:
            raise ValueError("SPDConv block_size must be at least 2")
        self.block_size = block_size
        self.conv = Conv(c1 * block_size * block_size, c2, k, 1, act=act)

    def forward(self, x):
        """Rearrange spatial samples into channels and project them."""
        height, width = x.shape[-2:]
        if height % self.block_size or width % self.block_size:
            raise ValueError(
                f"SPDConv requires dimensions divisible by {self.block_size}, got {(height, width)}"
            )
        return self.conv(torch.nn.functional.pixel_unshuffle(x, self.block_size))
