...
def parse_model(d, ch, verbose=True):
    ...
    cls_geometry_fuse = d.get("cls_geometry_fuse", False)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m is RGBColorAugment:
            ...
            c2 = ch[f[0]]
            args = [[ch[x] for x in f], *args]
        elif m is ASRMStructuralPrior:
            c2 = 1
            args = [[ch[x] for x in f], *args]
        elif m is SPGFusion:
            c2 = args[0]
            args = [[ch[x] for x in f], *args]
        elif m is ASRMP4SemanticVerifyResidual:
            c2 = ch[f[0]]
            args = [[ch[x] for x in f], *args]
        elif m in frozenset({P2ASRMFusion, InputASRMAuxiliaryAttention, InputASRMAttentionGuidedP2}):
