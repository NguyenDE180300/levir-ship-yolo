...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        # FTAL changes BCE classification targets after standard TAL assignment;
        # it remains opt-in so existing experiments are unaffected.
        ...
        if self.factorized_tal_target and (self.vfl is not None or bool(getattr(h, "cls_iou_target", False))):
            raise ValueError("Factorized TAL requires the default TAL-BCE classification loss.")
    ...
    def factorized_tal_cls_targets(
        ...
        """Sharpen standard TAL ranking for P2 positives assigned to small GTs."""
        epoch = int(getattr(self, "epoch", 0))
        if epoch < self.factorized_tal_warmup_start or not fg_mask.any():
            return target_scores
        ramp = 1.0
        if self.factorized_tal_warmup_end > self.factorized_tal_warmup_start:
            ramp = (epoch - self.factorized_tal_warmup_start) / (
                self.factorized_tal_warmup_end - self.factorized_tal_warmup_start
            )
        lam = self.factorized_tal_lambda * min(max(ramp, 0.0), 1.0)
        if lam <= 0:
            return target_scores

        out = target_scores.clone()
        positives = fg_mask.clone()
        if self.factorized_tal_p2_only:
            positives[:, n_p2:] = False
        eps = torch.finfo(target_scores.dtype).eps
        for batch_index in range(target_scores.shape[0]):
            for gt_index in target_gt_idx[batch_index, positives[batch_index]].unique():
                group = positives[batch_index] & (target_gt_idx[batch_index] == gt_index)
                if not group.any():
                    continue
                gt_box = gt_bboxes[batch_index, gt_index]
                if (gt_box[2:] - gt_box[:2]).clamp_min(eps).prod().sqrt() >= self.factorized_tal_s_max:
                    continue
                q = target_scores[batch_index, group]
                q_score = q.sum(-1)
                gt_box_grid = (gt_box / stride_tensor[group]).detach()
                iou = bbox_iou(
                    pred_bboxes[batch_index, group].detach(), gt_box_grid, xywh=False, CIoU=False
                ).squeeze(-1).clamp(0)
                q_new_score = iou.max().clamp_min(eps).pow(self.factorized_tal_tau) * (
                    q_score / q_score.max().clamp_min(eps)
                ).clamp(0, 1).pow(self.factorized_tal_kappa)
                scale = (q_new_score / q_score.clamp_min(eps)).unsqueeze(-1)
                q_new = torch.where(q > 0, q * scale, q)
                out[batch_index, group] = q + lam * (q_new - q)
        return out
